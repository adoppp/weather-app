...
const citiesSlice = createSlice({
    ...
    reducers: {
        ...
        addGeoCity(state, action: PayloadAction<{ city: string, coords?: { lat: number, lon: number}, index: boolean}>) {
            const indexExists = state.data.find(item => item.index === true);
...
export const { addCity, deleteCity, addGeoCity } = citiesSlice.actions;
